...
from dataset.dataloader_bert import BertDataset
...
NUM_CLASSES = classes_dict[DATASET_NO]
# BERT_MODEL_NAME = "bert-large-uncased"


# lrs = [1e-4, 1e-5, 3e-5, 5e-5]
lrs = [5e-5, 2e-5]
models = ["bert-large-uncased", "bert-base-uncased"]
for lr,BERT_MODEL_NAME in itertools.product(lrs,models):
    ...
    # train dataset
    train_dataset= BertDataset(tokenizer, max_length=100, split="train", dataset_no=DATASET_NO, oversampling_ratios=OVERSAMPLING_RATIOS)
    train_dataloader=DataLoader(dataset=train_dataset,batch_size=BATCH_SIZE)
    print(len(train_dataset), len(train_dataloader))

    # dev dataset
    dev_dataset= BertDataset(tokenizer, max_length=100, split="valid", dataset_no=DATASET_NO, oversampling=0)
    ...
    # model
    model=BERT(n_classes=NUM_CLASSES, bert_model_name=BERT_MODEL_NAME, d_model=D_MODEL).to(device)
    ...
    plt.legend()
    plt.savefig(f"plots/D{DATASET_NO}/{model_name}_train_losses_{EPOCHS}_{lr}_D{DATASET_NO}.pdf")

    plt.figure()
    plt.plot(torch.tensor(metrics["val_losses"]).to("cpu"), label="Val loss")
    plt.title(f"Epochs - {EPOCHS} Batch Size - {BATCH_SIZE} Le -{lr}")
    plt.legend()
    plt.savefig(f"plots/D{DATASET_NO}/{model_name}_val_losses_{EPOCHS}_{lr}_D{DATASET_NO}.pdf")
    ...
    with open(f"results/{BERT_MODEL_NAME}_{DATASET_NO}.txt", "a") as fp:
    ...
    ckpt_name = f"{BERT_MODEL_NAME}_{DATASET_NO}_{EPOCHS}_{lr}_{sum(OVERSAMPLING_RATIOS)}"
